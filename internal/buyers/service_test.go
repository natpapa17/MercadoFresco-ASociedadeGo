...
func TestCreate(t *testing.T) {
	...
	// 	mockBuyerRepository.On("LastID").Return(1, nil).Once()
	// 	mockBuyerRepository.On("GetByCode", mock.AnythingOfType("string")).Return(makeBuyer(), nil).Once()

	// 	_, err := service.Create(makeCreateParams())

	// 	assert.EqualError(t, err, "buyer code: valid_code is already in use")
	// })
	...
	t.Run("create_ok", func(t *testing.T) {
		...
		// 	Return(buyers.Buyer{}, errors.New("buyer code: valid_code is already in use")).
		// 	Once()
		mockBuyerRepository.
...
func TestUpdate(t *testing.T) {
	...
	// 	dbBuyer := makeBuyer()
	// 	dbBuyer.ID = 2
	// 	mockBuyerRepository.On("UpdateBuyerById", mock.AnythingOfType("int"), mock.AnythingOfType("string"), mock.AnythingOfType("string"), mock.AnythingOfType("string"), mock.AnythingOfType("string")).Return(dbBuyer, nil).Once()

	// 	_, err := service.UpdateBuyerById(makeUpdateParams())

	// 	assert.EqualError(t, err, "buyer id: valid_id is already in use")
	// })
