...
func TestCreateBuyer(t *testing.T) {
	...
	// t.Run("Should return an error and 400 status if Create from Buyers Service returns an Business Rule error", func(t *testing.T) {
	// 	mockBuyerService.On("Create", mock.AnythingOfType("string"), mock.AnythingOfType("string"), mock.AnythingOfType("string"), mock.AnythingOfType("string")).Return(buyers.Buyer{}, &buyers.BusinessRuleError{Err: errors.New("any_message")}).Once()
	// 	rr := httptest.NewRecorder()
	// 	req, _ := http.NewRequest(http.MethodPost, "/buyers", makeValidCreateBody())
	// 	r.ServeHTTP(rr, req)
